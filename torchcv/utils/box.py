...
def box_iou_ignore(box1, box2, labels):
    ...
    try:
      if any( ign_label ):    
        iou[:,ign_label] = inter[:,ign_label] / area1[:,None]
    except:
      import torchcv.utils.trace_error
      pdb.set_trace()
    # iou[:,ign_label] = inter[:,ign_label] / area2[ign_label]
...
def box_nms(bboxes, scores, threshold=0.5, top_k=200):
