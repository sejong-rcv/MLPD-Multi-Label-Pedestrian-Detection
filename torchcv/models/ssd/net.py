...
class SSDPed(nn.Module):
    ...
    def __init__(self, num_classes):
        super(SSDPed, self).__init__()
        self.num_classes = num_classes       

        self.extractor = VGG16ExtractorPed()
    ...
    def forward(self, x):
        ...
        for i, h in enumerate(hs):
            loc_pred = self.loc_layers[i](h)
            # loc_pred = self.loc_layers(x)
            loc_pred = loc_pred.permute(0,2,3,1).contiguous()
            loc_preds.append(loc_pred.view(loc_pred.size(0),-1,4))

            cls_pred = self.cls_layers[i](h)
...
class VGG16ExtractorPed(nn.Module):
    def __init__(self):
        ...
        ######## Feature map 1. ########
        ### ~ Conv3_3        
        vgg16_conv3 = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M']
        self.conv = CBRBlock(1, use_bn=True, cfg=vgg16_conv3)   ## Thermal only model
